...
export const TrainerBoard = ({ title, subtitle, trainers, championTrainer }: TrainerBoardProps) => {
  ...
  return (
    <div className="pt-32 pb-32 px-6 md:px-12 max-w-7xl mx-auto">
      <section className="mb-16">
        <h1 className="text-6xl md:text-8xl font-black font-headline tracking-tighter text-on-background mb-4 uppercase">{title} <span className="text-primary-container">{subtitle}</span></h1>
        <p className="max-w-2xl text-on-surface-variant font-body text-lg leading-relaxed">A curated breakdown of team compositions and strategy insights built for Omega Ruby Kaizo Origins.</p>
      </section>

      <div className="mb-20">
        <div className="flex items-center gap-4 mb-10">
          <h2 className="text-2xl font-black font-headline tracking-widest text-primary-container uppercase">{title}</h2>
      ...
      {championTrainer && (
        <section className="mb-20">
          <div className="flex items-center gap-4 mb-10">
            <h2 className="text-2xl font-black font-headline tracking-widest text-primary-container uppercase">Champion</h2>
            <div className="h-[1px] flex-1 bg-gradient-to-r from-primary-container/30 to-transparent"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 h-auto md:h-[600px]">
            <div className="md:col-span-8 bg-surface-container-high rounded-xl overflow-hidden relative group">
              <img
                src={championTrainer.imageUrl}
                alt={championTrainer.name}
              ...
              <div className="absolute bottom-10 left-10">
                <h3 className="text-6xl md:text-8xl font-black font-headline uppercase text-on-background tracking-tighter">{championTrainer.name}</h3>
            ...
            <div className="md:col-span-4 flex flex-col gap-6">
              <div className="flex-1 bg-surface-container-low p-8 rounded-xl border border-white/5 crimson-glow-card">
                ...
                <ul className="space-y-4">
                  {championTrainer.pokemon.map((p) => (
                    <li key={p.name} className="flex justify-between items-center pb-2 border-b border-white/5">
                      <span className={`font-bold ${p.isAce ? 'text-on-background' : 'text-on-surface-variant'}`} >{p.name}{p.isAce ? ' (Ace)' : ''}</span>
                      {p.isAce ? <Star size={14} className="text-primary-container" fill="currentColor" /> : <span className="text-[10px] font-label opacity-50 uppercase">Tactical</span>}
                    </li>
                  ))}
                </ul>
              </div>
